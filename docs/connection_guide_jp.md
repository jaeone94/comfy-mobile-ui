...
- **サーバー設定 > 認証**で **ComfyUI-Login** を選択します。
- ComfyUIコンソールの `For direct API calls, use token=` の後に表示される値をコピーし、**APIトークン**に貼り付けます。
...
> [!WARNING]
> APIトークンはパスワードと同様に扱ってください。信頼できないネットワークではHTTPSを使用してください。
