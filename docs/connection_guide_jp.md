...
### 4. ComfyUI-Loginを使用する場合
サーバーが [ComfyUI-Login](https://github.com/liusida/ComfyUI-Login) で保護されている場合：
- 最も簡単な方法: トークン入力欄の下にある**サーバーからトークンを取得**をタップします。`<サーバー>/comfymobile/api/auth/token` が開き、ComfyUI-Login のパスワードでログインするとコピーボタン付きでトークンが表示されます。コンソールもファイルアクセスも不要です。
- または ComfyUI のコンソールで `For direct API calls, use token=` の後に出力された値をコピーしてください。起動時に一度だけ出力されるため、再起動後は見逃しやすいです。
- 通常のパスワードではなく、生成されたAPIトークンを使用してください。トークンは `<ComfyUI>/login/PASSWORD` の1行目でも確認できます。
- 既定ではトークンは現在のブラウザセッションにのみ保持されます。**この端末でログインを保持**を有効にするとトークンが端末に保存され、タブを閉じても再入力が不要になります。その場合トークンはブラウザデータのバックアップにも含まれます。共用端末ではオフのままにしてください。

> [!WARNING]
> API トークンはパスワードと同様に扱ってください。信頼できないネットワークでは HTTPS を使用してください。

> [!IMPORTANT]
> ComfyUI は値を付けずに `--enable-cors-header` で起動してください。特定のオリジンを指定すると
> ComfyUI-Login はその `Origin` ヘッダーを持つリクエストでしかトークンを受け付けなくなりますが、
> 画像やメディアのリクエストは `Origin` を送らないため、プレビューやサムネイルが読み込めません。
